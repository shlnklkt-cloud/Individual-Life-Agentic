...
import { AppState, UserData, UnderwritingDecision, Message, AgentName } from './types';
...
const AGENT_REGISTRY: Record<AgentName, { role: string; icon: React.ReactNode; color: string; accent: string }> = {
  'Orchestrator Agent': { role: 'Workflow Lead', icon: <LayoutGrid className="w-4 h-4" />, color: 'bg-black', accent: 'text-black' },
  ...
  'Data Integrity & Disclosure Validation': { role: 'KYC Expert', icon: <ClipboardCheck className="w-4 h-4" />, color: 'bg-[#B11226]', accent: 'text-red-300' },
  'Fraud & Anomaly Detection': { role: 'Risk Auditor', icon: <Fingerprint className="w-4 h-4" />, color: 'bg-black', accent: 'text-red-500' },
  'Med Triage': { role: 'Clinical Filter', icon: <Stethoscope className="w-4 h-4" />, color: 'bg-black', accent: 'text-emerald-400' },
  'Med Evidence Collection': { role: 'Data Harvester', icon: <FlaskConical className="w-4 h-4" />, color: 'bg-black', accent: 'text-teal-400' },
  'Med Risk Interpretation': { role: 'Clinical Pathologist', icon: <Brain className="w-4 h-4" />, color: 'bg-black', accent: 'text-violet-400' },
  'Lifestyle & Medical Interaction': { role: 'Metabolic Specialist', icon: <Zap className="w-4 h-4" />, color: 'bg-black', accent: 'text-amber-400' },
  'Mortality Scoring & Risk Aggregation': { role: 'Actuarial Scientist', icon: <Dna className="w-4 h-4" />, color: 'bg-black', accent: 'text-red-600' },
  'Pricing & Terms Recommendation': { role: 'Premium Strategist', icon: <Coins className="w-4 h-4" />, color: 'bg-black', accent: 'text-yellow-400' },
  'Explainability & Adverse Action': { role: 'Compliance Officer', icon: <Search className="w-4 h-4" />, color: 'bg-black', accent: 'text-cyan-400' },
  'Human Underwriter Collaboration': { role: 'Bridge Agent', icon: <UserCheck className="w-4 h-4" />, color: 'bg-black', accent: 'text-orange-400' },
  'Final Underwriting Decision': { role: 'Executive Sign-off', icon: <Scale className="w-4 h-4" />, color: 'bg-black', accent: 'text-white' },
  'Finley': { role: 'Finance Agent', icon: <Wallet className="w-4 h-4" />, color: 'bg-black', accent: 'text-red-500' },
  'Lyra': { role: 'Issuance Officer', icon: <FileCheck className="w-4 h-4" />, color: 'bg-black', accent: 'text-red-500' }
};
...
const App: React.FC = () => {
  ...
  const handlePolicyIssuance = () => {
    setIsIssuing(true);
    setTimeout(() => {
      const id = `DGL-${Math.floor(Math.random() * 1000000).toString().padStart(6, '0')}`;
      setPolicyId(id);
      setIsIssuing(false);
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans selection:bg-red-100 selection:text-[#B11226]">
      <header className="bg-[#B11226] border-b border-white/20 sticky top-0 z-[100] h-16 shadow-xl">
        <div className="max-w-6xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-4 bg-black/20 px-5 py-2 rounded-2xl border border-white/10">
            <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${transferring ? 'bg-amber-400' : 'bg-emerald-400'}`}></div>
            <div className="flex items-center gap-3">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-white shadow-sm ${AGENT_REGISTRY[activeAgent].color} border border-white/20`}>
                  ...
                  if (isHandover) {
                    return (
                      <div key={m.id} className="flex justify-center my-6 animate-in fade-in zoom-in duration-500">
                        <div className="flex items-center gap-3 px-6 py-2 bg-black border border-white/10 rounded-full text-[9px] font-black text-red-500 tracking-[0.15em] uppercase shadow-2xl">
      ...
      <footer className="py-8 text-center text-black text-[9px] border-t border-black/5 bg-white/50 backdrop-blur-md">
        <div className="flex items-center justify-center gap-6 font-black uppercase tracking-[0.2em]">
           <span className="text-[#B11226]">&copy; 2024 DiaGuard Life</span>
           <span className="text-black/10">|</span>
           <span>GDPR Compliant</span>
           <span className="text-black/10">|</span>
           <span>SEC-Verified Ledger</span>
        </div>
      </footer>
    </div>
  );
